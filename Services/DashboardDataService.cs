...
namespace NFCVeraConstrucoes.Services
{
    public class DashboardDataService
    {
        ...
        {
            var files = Directory.EnumerateFiles(_xmlDirectory, "*.xml", SearchOption.TopDirectoryOnly);
            var notas = files
            ...
            var valorMedioNotas = totalNotas > 0 ? Math.Round(notas.Average(n => n.ValorTotal), 2) : 0m;
        ...
        private DashboardNota? TryParseNotaFiscal(string filePath)
        {
            try
            {
                ...
                var clienteNome = dest?.Element(Ns + "xNome")?.Value ?? dest?.Element(Ns + "CPF")?.Value ?? dest?.Element(Ns + "CNPJ")?.Value ?? string.Empty;
                var id = infNFe.Attribute("Id")?.Value ?? Path.GetFileNameWithoutExtension(filePath);

                var dataEmissao = DateTime.TryParse(dhEmiText, null, DateTimeStyles.AdjustToUniversal, out var parsedDate)
